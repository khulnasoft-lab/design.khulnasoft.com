...
function findMatch(arr1, arr2) {
  const matchingElements = arr1.filter(element => arr2.includes(element));
  const matchingPositions = [];
  matchingElements.forEach(element => {
    const arr1Pos = arr1.indexOf(element);
    const arr2Pos = arr2.indexOf(element);
...
export default function(req, res, next) {
  ...
  const urlParts = stripSlash(reqPath).split('/');

  // This a variable to track all evaluated matched directions.
  const allMatchedRedirections = [];
  ...
  redirects.forEach(r => {
    ...
    if (match && arraysEqual(redirectFromParts, urlParts)) {
      ...
      match &&
      urlParts.length > 1 &&
      redirectNewParts.length === 1 &&
      redirectFromParts.length === 1
    ) {
      // We simply copy the urlParts array
      const targetParts = [...urlParts];
