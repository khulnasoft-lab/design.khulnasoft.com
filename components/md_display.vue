...
function makeAdmonitions(html, type) {
  ...
  if (type === ADMONITION_TODO) {
    return html.replace(
      pattern,
      `$1<span class="admonition admonition--${type} app-styles gl-rounded-base"><div class="gl-display-flex gl-align-items-center">${todoIconHtml}<span class="gl-mr-2 admonition-type">${typeCapitalized}: </span></div><span class="admonition-text">$2</span>${createIssueHtml}</span>$3`,
    );
  }
  return html.replace(
    pattern,
