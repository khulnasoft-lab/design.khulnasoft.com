<template>
  <div class="content limited m-t-7 m-b-7">
    <div class="component md">
      ...
      <p><a href="#instant-feedback">Instant feedback</a>, on the other hand, is giving user immediate positive feedback without waiting for the return of data from the server, expecting that the change they made will be successfully saved. It has a positive effect on the perceived speed of the application.</p>

      <h2 id="saving-progress">Saving progress</h2>
      <p>The guideline for choosing between the two methods of saving progress is: introduce automatic saving of changes if you believe that it will improve the user’s experience in the given case, if you think it may add confusion to the process, default to saving manually. Automatically saving changes can be sensitive and comes with its own set of guidelines. Consider those guidelines when deciding between the two methods.</p>
      ...
      <p>As we move towards using both methods for saving changes, it makes sense to add a safety measure when using the manual method. The user might just experience auto-saving on another page but on the current page they need to manually save their changes. The users can’t know  which of the two methods is being used as there are no indicators in the UI, so it's possible that they make changes and try to navigate off the page (expecting that the changes were saved automatically). In such cases we should track the changes made and wether they were saved or not. If they weren’t saved, a <a href="components/modals">modal</a> should appear asking: “Your changes are not saved. Do you want to save them now?” and offer “Yes, save changes” as the primary option and “No, cancel” as the secondary one.</p>
      ...
      <ol>
        ...
        <li>Inform the user with a toast message saying “Saving x changes failed. Retrying…” with the option to retry manually. This message should remain visible until the changes are successfully saved.</li>
      </ol>
