<template>
  <div class="content limited m-t-7 m-b-7">
    <div class="component md">
      ...
      <p>Narrowing down a large list of results as a user types can create performance issues. Consider using the Search by confirmation pattern if performance is a concern.</p>
      <p class="todo">Todo: Add live component block with code example (search by typing)</p>
      <h3>Search by confirmation</h3>
      <p>This pattern is visually different from the one above because users interact with it differently. The magnifying glass icon is inside the button which triggers the search action. The presence of the button indicates the need for confirmation to trigger the search. Pressing “Enter” also triggers the action.</p>
      ...
      <h3>Clear icon</h3>
      <p>A clear icon appears in the right corner when the search box contains content and there is no search activity happening in the background (when it’s replaced with a spinner). It helps users clear the content with a single click. After removing the content it disappears and focus is placed on the input field. The clear icon is mandatory for search patterns as they are expected to change in content often by users refining or clearing their search phrases.</p>
      ...
      <p>Don’t use ellipsis (…) in the placeholder unless it’s truncated or the text is incomplete (see <a href="/content/punctuation">Punctuation</a>). For example, “Search” or “Search labels” don’t need an ellipsis, but “Search and jump to…” uses one because the text is incomplete.</p>
      <p>Even though the search patterns can be used for either filtering or searching for information, always try to use the word “Search” for the placeholder. This is another small detail that makes the pattern easier to recognize. As an exception, it’s ok to use another word if it makes the interaction and the experience clearer.</p>
      <h3>Positioning</h3>
