...
Using a keyboard is a fundamental aspect of accessibility. In fact, many other technologies and hardware emulate keyboard functionality. Users with a wide range of disabilities — or no disability at all — rely on a keyboard. When testing with a keyboard keep in mind that all interactive elements should be accessible with the `Tab` key.
...
- Only elements that the user can interact with via the keyboard are focusable.
- Focus order follows the reading order.
- With widgets such as dropdowns:
  ...
  - `Enter` accepts the selected value and closes the popup. Focus remains on the widget.
  - `Escape` closes the popup and focus remains on the widget.
