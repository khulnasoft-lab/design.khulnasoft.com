...
Buttons are ordered by importance from right to left. This means the primary action of the current context should be represented with the rightmost button. Generally this means that primary buttons are on the right followed by secondary buttons, and so on. Buttons with dismissive actions will generally be on the left and buttons with affirmative actions are on the right. Depending on the context, the primary, affirmative action may be destructive.
