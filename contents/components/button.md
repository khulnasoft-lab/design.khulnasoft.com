...
|Danger|A final destructive action that can't be undone or has potentially detrimental consequences. In a long list of items where each contains a destructive button, use the `default` variant to avoid having many danger buttons overwhelm the page. Also, ensure the resulting confirmation does contain the `danger` button variant to confirm the final removal or deletion.|The **Delete account** button when deleting an account.|
