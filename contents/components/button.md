...
- **Emoji**: An emoji button allows a user to react to content with a specific emoji that is displayed with a count of how many times that reaction has been used. The button uses the medium size and a selected state visually indicates when the current user has added the reaction.
