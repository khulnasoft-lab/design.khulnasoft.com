...
|Danger|A final destructive action that can't be undone or has potentially detrimental consequences. In long lists of items with destructive buttons, use the `default` variant instead to reduce color overload. However, ensure the final confirmation contains the `danger` button variant to convey the final removal or deletion.|The **Delete account** button when deleting an account.|
