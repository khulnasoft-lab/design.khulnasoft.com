...
- When the date range has a limit on the number of selectable days, indicate that in the UI with text summarizing the number of days currently selected, and an [info icon](https://gitlab-org.gitlab.io/gitlab-svgs/?q=~information-o) specifying the date range limit.
