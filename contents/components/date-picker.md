...
When using the input field, the date format defaults to GitLab's and is expected in ISO 8601 format. The input field only accepts the numeric characters `0123456789` and the *en dash* special character `-`. No other characters can be entered in the input field.
