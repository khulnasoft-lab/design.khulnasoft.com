...
1. **Inline text link**: Link that appears within body text. In order to distinguish a linked reference from surrounding content, a link within body copy must be underlined. For example, referencing an issue in a merge request description where "#3126" has the primary function of linking to the referred issue. Inline text links can be user-generated, for example in markdown content. They can also be part of the system UI, like a link in a paragraph of text.
