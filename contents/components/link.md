...
- An inline or mention link uses a darker blue to ensure sufficient contrast on surfaces that use a `50` or `100` step from the [UI color palette](/product-foundations/color), while a UI or meta link uses a more vibrant blue and relies on CSS blend modes to increase contrast on different surfaces.
