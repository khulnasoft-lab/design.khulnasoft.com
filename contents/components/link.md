...
- As mentioned in the [categories](#categories) section above, the primary purpose of a meta link is first and foremost as content — that it's linked should neither be necessary or consequential in context. Therefore, the visual emphasis that would come from an underline is not used, even if the link is within a string of unlinked text, but it is present for hover. This is an intentional decision to visually simplify certain sections in the UI and may change in the future. More discussion and consideration can be found [this issue](https://gitlab.com/gitlab-org/gitlab-services/design.gitlab.com/-/issues/1266#note_871929550).
