...
* When child-level settings are enabled from a parent level. In this scenario, a feature may be disabled or replaced with a read-only equivalent. There should be text explaining that the setting is configured at the parent level. For example, a lock icon may be displayed, along with a tooltip, explaining that this setting is configured at a parent level.
