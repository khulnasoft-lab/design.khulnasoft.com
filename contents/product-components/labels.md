...
Labels should not be confused with [badges](/product-components/badge). Badges highlight more generic object metadata that are not categories.
...
In most places, a label's description is shown in a [tooltip](/product-components/tooltip) when hovering the label. But if the label's description appears on the page with no need for user interaction, do not show it in a tooltip.
