...
* Use a single checkbox when the result of a checked/unchecked state is **not** effective or noticeable immediately, and the user needs a confirmation of it being saved. (Use a [toggle](/product-components/toggle) when a state change has an immediate effect.)
...
* [Help text](/product-components/forms#help-text) can be added below the checkbox label or as a paragraph below the group.
